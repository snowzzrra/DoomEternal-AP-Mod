...
3. **Offline Local Assembly (Fallback / Reproducibility)**: Maintainers can also assemble or inspect release archives locally using `scripts/release/assemble_ci_artifact.py` against a downloaded handoff bundle and local canonical room resources (`packaging/room_resources/v0.5.2/` or `build/release/client/resources/`).
...
2. Copy the regenerated canonical room resources to the versioned packaging directory:
   ```bash
   cp build/release/client/resources/base_mod.zip packaging/room_resources/v0.5.2/
   cp build/release/client/resources/room_payloads.zip packaging/room_resources/v0.5.2/
   cp build/release/client/resources/room_payload_manifest.json packaging/room_resources/v0.5.2/
   ```
